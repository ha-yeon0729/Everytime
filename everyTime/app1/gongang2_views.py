...
def gongang2(request):
    if True:
        ...
        print("AAAAA")
        selected=excel_db.objects.filter(my_name=request.session['name'])
        #selectDB=selected.friend_name
        #selected_DB=excel_db.objects.filter(friend_name=selected)
        #print(selected_DB)
        #print(selected_DB.friend_name)
        ...
        return render(request,"gongang2.html",{'select':selectDB,})
